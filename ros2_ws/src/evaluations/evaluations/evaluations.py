...
class PathEvaluationNode(Node):
    ...
    def compute_orientation_error(self):
        ...
        robot_heading_deg = robot_yaw_deg - 90.0

        robot_heading_deg = robot_heading_deg % 360
        expected_heading = self.expected_heading_deg % 360

        error = abs(robot_heading_deg - expected_heading)
        error = min(error, 360 - error)
        self.get_logger().info(
            f"Orientation check: robot={robot_heading_deg:.2f}°, "
            f"expected={expected_heading:.2f}°, error={error:.2f}°"
        )
