...
struct LocalPlanner : public rclcpp::Node
{
    ...
    rclcpp::Publisher<unitycustommsg::msg::SocialMetrics>::SharedPtr metrics_publisher;
        ...
        : Node("local_planner"), unity_to_pixel(), pixel_to_unity()
    {
        ...
        metrics_publisher = this->create_publisher<unitycustommsg::msg::SocialMetrics>(
            "/social_metrics", 10);
        global_trajectory_publisher =
            this->create_publisher<unitycustommsg::msg::Point2DArray>(
        ...
        timer_ =
            this->create_wall_timer(std::chrono::milliseconds(100),
                                    std::bind(&LocalPlanner::publishSFMPath, this));
    ...
    void publishSFMPath()
    {
        ...
        social_force::FreeWalkOutput free_walk_output = local_planner->free_walk();
        auto &trajectory = free_walk_output.trajectory;

        // TODO VEFAK send stuff
        // free_walk_output.report.min_distance_from_static;
        // free_walk_output.report.min_distance_from_dynamic;
        // free_walk_output.report.cumulative_social_force;
        // free_walk_output.report.cumulative_static_force;
        // free_walk_output.report.cumulative_dynamic_force;
        // free_walk_output.report.social_work;

        std::cout << "min_distance_from_static:" << free_walk_output.report.min_distance_from_static << std ::endl;
        std::cout << "min_distance_from_dynamic:" << free_walk_output.report.min_distance_from_dynamic << std ::endl;
        std::cout << "cumulative_social_force:" << free_walk_output.report.cumulative_social_force << std ::endl;
        std::cout << "cumulative_static_force:" << free_walk_output.report.cumulative_static_force << std ::endl;
        std::cout << "cumulative_dynamic_force:" << free_walk_output.report.cumulative_dynamic_force << std ::endl;
        std::cout << "social_work:" << free_walk_output.report.social_work << std ::endl;

        // End measuring time for the control loop
        auto loop_end = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> loop_duration = loop_end - loop_start;
        std::cout << "Published path length: " << trajectory.path.size()
                  << std::endl;
        if (trajectory.path.size() > 0)
        {
            std::cout << "Start: " << trajectory.path[0]
                      << " Finish: " << trajectory.path.back() << std::endl;
            for (auto &point : trajectory.path)
                std::cout << point << " ";
            std::cout << std::endl;
        }

        std::cout << "Control Loop Time: " << loop_duration.count() << " seconds"
                  << std::endl;
        free_walk_output.trajectory.adjust_coordinates(pixel_to_unity);
        std::vector<geometry::Point> path = trajectory.path;

        //  Create and populate Point2DArray message
        auto array_message = unitycustommsg::msg::Point2DArray();

        // Populate array_message with the path points
        for (const auto &point : path)
        {
            unitycustommsg::msg::Point2D point_msg;
            point_msg.x = point.x;
            point_msg.y = point.y;
            array_message.points.push_back(point_msg);
        }

        trajectory_publisher->publish(array_message);

        // Prepare and publish the social metrics message
        auto metrics_msg = unitycustommsg::msg::SocialMetrics();
        metrics_msg.min_distance_from_static = free_walk_output.report.min_distance_from_static;
        metrics_msg.min_distance_from_dynamic = free_walk_output.report.min_distance_from_dynamic;
        metrics_msg.cumulative_social_force = free_walk_output.report.cumulative_social_force;
        metrics_msg.cumulative_static_force = free_walk_output.report.cumulative_static_force;
        metrics_msg.cumulative_dynamic_force = free_walk_output.report.cumulative_dynamic_force;
        metrics_msg.social_work = free_walk_output.report.social_work;
        metrics_publisher->publish(metrics_msg); // Second message

        // Optionally print debug info
        std::cout << "Published path length: " << path.size() << std::endl;
    }
